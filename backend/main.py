...
async def admin_clean_db(request: Request):
    ...
    async with stats_store.get_db() as db:
        ...
        async with db.execute("SELECT id FROM prs WHERE pr_number = ?", (keep_pr,)) as cursor:
            keep_rows = await cursor.fetchall()
        keep_ids = [row[0] for row in keep_rows]

        if keep_ids:
            placeholders = ",".join("?" * len(keep_ids))
            del_issues = await db.execute(f"DELETE FROM issues WHERE pr_id NOT IN ({placeholders})", keep_ids)
            del_prs = await db.execute(f"DELETE FROM prs WHERE id NOT IN ({placeholders})", keep_ids)
        else:
...
@app.post("/api/admin/mock_review")
async def admin_mock_review(request: Request):
    """
    Admin endpoint: Inject a perfectly calibrated mock review for QA/testing.
    Accepts JSON: {"pr_number": 161, "high": 1, "medium": 1, "low": 1}
    """
    body = await request.json() if request.headers.get("content-type") == "application/json" else {}
    pr_number = body.get("pr_number", 161)
    high = body.get("high", 1)
    medium = body.get("medium", 1)
    low = body.get("low", 1)

    issues = []
    # Generate High issues
    for i in range(high):
        issues.append({
            "severity": "HIGH",
            "type": "security",
            "title": f"Mock Security Vulnerability #{i+1}",
            "description": "This is a pre-calibrated high-severity vulnerability to verify status cards.",
            "file": "backend/demo_helper.py",
            "line": 15 + i
        })
    # Generate Medium issues
    for i in range(medium):
        issues.append({
            "severity": "MEDIUM",
            "type": "bug",
            "title": f"Mock Logic Bug #{i+1}",
            "description": "This is a pre-calibrated medium-severity logic warning to verify status cards.",
            "file": "backend/demo_helper.py",
            "line": 20 + i
        })
    # Generate Low issues
    for i in range(low):
        issues.append({
            "severity": "LOW",
            "type": "quality",
            "title": f"Mock Code Quality Note #{i+1}",
            "description": "This is a pre-calibrated low-severity code quality tip to verify status cards.",
            "file": "backend/demo_helper.py",
            "line": 25 + i
        })
    ...
    logger.info(f"[ADMIN] Injected mock review for PR #{pr_number} (High={high}, Med={medium}, Low={low})")
    return {
        "status": "mocked",
        "pr_number": pr_number,
        "metrics": {"high": high, "medium": medium, "low": low},
        "decision": decision
    }
